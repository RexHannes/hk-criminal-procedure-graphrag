...
async function ensureCollection(env, collectionName, dimension) {
  try {
    ...
    if (currentSize !== dimension) {
      throw new Error(`${collectionName} vector size ${currentSize} != requested ${dimension}`);
    }
  ...
  for (const paragraph of paragraphPayload.paragraph_cards || []) {
    ...
    paragraphPoints.push({
      ...
      payload: {
        ...
        vector_scope: "sedition_public_expression_tree_gap_pilot_v1",
        ...
        issue_tags: ["sedition", "public_expression", "public_order"],
        ...
        indexed_text_preview: paragraph.text.slice(0, 500),
      },
    });
  }

  const propositionPoints = [];
  for (const card of propositionPayload.proposition_cards || []) {
