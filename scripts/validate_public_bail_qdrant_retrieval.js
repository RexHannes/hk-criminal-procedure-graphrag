...
const LINEAGE_QUERY = "Tong Ying Kit limited by Lai Chee Ying CFA Article 42 bail";
...
const lineagePayload = runQuery(LINEAGE_QUERY);
const errors = [];
...
const lineageHits = lineagePayload.hits || [];
...
const lineageBailHits = lineageHits.filter(hit => hit.batch_id === "criminal_bail_public_batch_v1");
...
const lineageIds = lineageBailHits.map(hit => hit.proposition_id);
...
assert(undertakingIds.includes("prop_lai_cfi_2020_tailored_undertaking_sufficient_p33"), "expected Lai CFI tailored undertaking proposition in targeted undertaking query", errors);
...
assert(lineageIds.includes("prop_lai_2021_tong_limited_p72"), "expected Tong-limited lineage proposition in targeted lineage query", errors);
...
assert(lineageBailHits.every(hit => hit.answer_layer_status === "candidate_only"), "lineage bail hits must remain candidate_only", errors);
