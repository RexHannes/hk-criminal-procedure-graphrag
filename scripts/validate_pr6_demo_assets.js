...
const EXPECTED = [
  ...
  },
  {
    ...
    mustNot: ["no uploaded evidence has been parsed", "answer_safe: true"],
  },
  {
    ...
    mustNot: ["demo_supported", "answer_safe: true"],
  },
];
...
assert(viewer.includes("${esc(String(productMode.mode))}"), "viewer should display exact product-mode token", errors);
...
for (const expected of EXPECTED) {
  ...
  assert(content.includes("needs_lawyer_review: true"), `${expected.id}: lawyer-review boundary missing`, errors);
  assert(content.includes("answer_safe: false"), `${expected.id}: answer_safe false missing`, errors);
