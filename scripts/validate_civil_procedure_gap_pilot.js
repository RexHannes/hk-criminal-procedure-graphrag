...
const PILOT_DIR = path.join(ROOT, "data", "legal_ingest", "tree_gap_pilots", "civil_procedure_inconsistent_pleadings_v1");
const DOMAIN_DIR = path.join(ROOT, "data", "legal_domain_packs", "demo_maps", "civil_procedure_hk");
...
function loadNodeIds() {
  ...
  for (const file of fs.readdirSync(nodesDir).filter(name => name.endsWith(".json"))) {
    ...
    for (const node of payload.nodes || []) {
      ...
      if (node.id) ids.add(`civil_procedure_hk.${node.id}`);
    }
  }
  return ids;
}
...
function main() {
  ...
  const knownNodeIds = loadNodeIds();
  ...
  if (manifest.domain_id !== "civil_procedure_hk") errors.push("manifest_domain_mismatch");
  ...
  if (parseReport.rejected_count !== 1) errors.push("expected_one_pending_pinpoint_rejection");
  if (!String(parseReport.rejected?.[0]?.reason || "").includes("paragraph_pinpoint_pending")) errors.push("expected_pending_pinpoint_rejection_reason");
  ...
  for (const source of manifest.sources || []) {
    ...
    if (!/^https:\/\/www\.hklii\.hk\//.test(source.source_url_or_path || "")) errors.push(`${source.source_id}:unexpected_source_url`);
  }
  ...
  const report = {
    validator: "civil_procedure_gap_pilot_v1",
    ...
    rejected_count: parseReport.rejected_count,
    errors,
  };
