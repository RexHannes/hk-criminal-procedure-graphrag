...
Generated for the `pr6-production-release` branch after the Part 1 two-vertical source-grounding work.
...
authority from user facts and uploaded evidence, and exposes public paragraph-linked
authority for research-prototype analysis while professional certification remains later HITL.
```
...
- Public paragraph proof is the current prototype gate. Lawyer review is quiet HITL metadata for later certification, not a blocker for research retrieval.
- Prototype metadata remains `answer_mode=research_prototype`, `lawyer_review_status=unreviewed`, and `professional_advice_certified=false`.
